    ...
    for detection_number, box_index in enumerate(ordered_indexes, start=1):
        ...
        try:
            ...
            if best_mask.any():
                ...
                green_overlay[best_mask] = (0, 255, 0)
                mask_overlay = cv2.addWeighted(mask_overlay, 1.0, green_overlay, 0.35, 0)

                y_indices, x_indices = np.where(best_mask)
                y_min, y_max = y_indices.min(), y_indices.max()
                x_min, x_max = x_indices.min(), x_indices.max()
                sam_crop = image[y_min:y_max + 1, x_min:x_max + 1]
        except RuntimeError as error:
            print(f"SAM2 failed for detection D{detection_number}: {error}")

        decoded_by_value = []
        preview_crop = sam_crop
        for padding in decode_paddings:
            ...
            for value in decode_barcode_candidates(yolo_crop, sam_crop):
        ...
        for value, padding in decoded_by_value:
            ...
            barcode_results.append({
                ...
                "bbox": [x1, y1, x2, y2],
                "source": f"YOLO/SAM D{detection_number} padding {padding}",
            })
